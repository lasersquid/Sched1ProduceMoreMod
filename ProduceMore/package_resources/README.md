...
Tired of waiting 8 hours for lab ovens? Configure station speed, employee work animation speed, walk speed, stack limits, and more.
...
The configuration file is ProduceMoreSettings.cfg, and is generated in Schedule I's UserData directory when the mod is run for the first time. The recommended method of modifying settings is to use a mod like ModsApp by k0. Otherwise, it can easily be edited by hand with any text editor (eg, Notepad). The default mod configuration is to use the same stack sizes and station speeds as the unaltered game, with unaltered employee walk and animation speeds.
